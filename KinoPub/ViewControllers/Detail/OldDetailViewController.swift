...
class OldDetailViewController: UIViewController, SideMenuItemContent {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem.init(
            image: UIImage(named: "Share-50")?.withRenderingMode(.alwaysTemplate), style: .plain,
            target: self, action: #selector(shareLink(_:)))  // UIBarButtonItem.init(barButtonSystemItem: .action, target: self, action: #selector(shareLink))
    ...
    override func viewDidLayoutSubviews() {
        ...
        configOffset()
        let frame = CGRect(
            x: headerView.bounds.origin.x, y: headerView.bounds.origin.y, width: view.bounds.width,
            height: headerView.bounds.height)
        headerImageView.frame = frame
        headerBlurImageView.frame = frame

        buttonView.center = CGPoint(
            x: headerImageView.frame.width / 2, y: headerImageView.frame.height / 2)
    ...
    func configHeaderView() {
        ...
        playButton.addTarget(
            self, action: #selector(playButtonAction), for: UIControl.Event.touchUpInside)
        let longGesture = UILongPressGestureRecognizer(
            target: self, action: #selector(playButtonLongTap(_:)))
        ...
        if model.mediaItem.url != nil {
            ...
            if model.watchingTime > 0 {
                mediaManager.playVideo(
                    ...
                    userinfo: ["watchingTime": TimeInterval(model.watchingTime)])
            } else {
    ...
    @IBAction func tapInWatchlistButton(_ sender: UIButton) {
        if model.item?.type == ItemType.shows.rawValue
            ...
            logViewsManager.changeWatchingStatus(
                id: model.item?.id ?? 0, video: nil, season: 0, status: nil)
        }
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if offset < 0 {
            ...
            headerTrasform = CATransform3DScale(
                headerTrasform, 1.0 + headerScaleFactor, 1.0 + headerScaleFactor, 0)
        ...
        // SCROLL UP/DOWN ----------
            ...
            headerTrasform = CATransform3DTranslate(
                headerTrasform, 0, max(-offsetHeaderStop, -offset), 0)

            // ---------- Label
            let alignToNameLabel =
                -offset + ruTitleLabel.frame.origin.y + headerView.frame.height + offsetHeaderStop
            headerLabel.frame.origin = CGPoint(
                x: headerLabel.frame.origin.x,
                y: max(alignToNameLabel, distanceWLabelHeader + offsetHeaderStop))

            // ---------- Blur
            headerBlurImageView?.alpha = min(
                1.0, (offset - alignToNameLabel) / distanceWLabelHeader)
        ...
        segmentTransform = CATransform3DTranslate(
            segmentTransform, 0, max(segmentViewOffset, -offsetHeaderStop), 0)

        segmentedView.layer.transform = segmentTransform

        // Set scroll view insets just underneath the segment control
        tableView.scrollIndicatorInsets = UIEdgeInsets(
            top: segmentedView.frame.maxY, left: 0, bottom: 0, right: 0)
    }
}
...
extension OldDetailViewController: VideoItemModelDelegate {
    ...
    func didUpdateItem(model: VideoItemModel, error: Error?) {
        if let error = error {
            ...
            if let season = model.mediaItems.first?.season,
                let number = model.mediaItems.first?.video {
