        ...
        form +++ Section("ОБЩЕЕ")
            ...
            <<< ButtonRow { (row: ButtonRow) in
                ...
                row.presentationMode = PresentationMode.show(
                    controllerProvider: ControllerProvider.callback(builder: {
            ...
            +++ Section(
                header: "КАСТОМНЫЙ ПЛЕЕР",
                footer: "Данные настройки применимы только для кастомного плеера."
            ) {
                $0.hidden = .function(
                    ["Custom"],
                    { form -> Bool in
            ...
            <<< ButtonRow { (row: ButtonRow) in
                ...
                row.presentationMode = PresentationMode.show(
                    controllerProvider: ControllerProvider.callback(builder: {
                        ...
                        thanksVC?.url =
                            "https://raw.githubusercontent.com/hintoz/KinoPub-iOS/master/CHANGELOG.md"
                        thanksVC?.titleText = ""
                        return thanksVC!
                    }),
                    onDismiss: { (vc) in
            ...
            +++ Section()
            <<< ButtonRow("Support") { (row: ButtonRow) in
