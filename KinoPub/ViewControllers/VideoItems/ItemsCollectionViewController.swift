...
class ItemsCollectionViewController: ContentCollectionViewController, SideMenuItemContent {
    ...
    func configFilterButton() {
        filterButton?.image =
            model.filter.isSet ? UIImage(named: "Filters Fill") : UIImage(named: "Filters")
    }
    ...
    static func storyboardInstance() -> ItemsCollectionViewController? {
        ...
        return storyboard.instantiateViewController(withIdentifier: String(describing: self))
            as? ItemsCollectionViewController
    }

    @objc func tap(sender: UITapGestureRecognizer) {
        guard (searchControllerNew.isActive ? model.resultItems.count : model.videoItems.count) > 0
        else { return }
        if let indexPath = self.collectionView?.indexPathForItem(
            ...
            if let cell = collectionView?.cellForItem(at: indexPath) as? ItemCollectionViewCell {
                if let image = cell.posterImageView.image {
                    showDetailVC(
                        with: searchControllerNew.isActive
                            ? model.resultItems[indexPath.row] : model.videoItems[indexPath.row],
                        andImage: image)
                }
            }
        }
...
extension ItemsCollectionViewController {
    ...
    override func collectionView(
        ...
        return (searchControllerNew.isActive ? model.resultItems.count : model.videoItems.count)
    ...
    override func collectionView(
        ...
        guard
            indexPath.row
                < (searchControllerNew.isActive ? model.resultItems.count : model.videoItems.count)
        else {
            let cell =
        ...
        cell.set(
            item: searchControllerNew.isActive
                ? model.resultItems[indexPath.row] : model.videoItems[indexPath.row])
        return cell
    }
    ...
    func paginableBehavior(
        ...
        if searchControllerNew.isActive {
            ...
            model.loadSearchItems(
                searchControllerNew.searchBar.text!, iOS11: true,
                { (resultCount) in
