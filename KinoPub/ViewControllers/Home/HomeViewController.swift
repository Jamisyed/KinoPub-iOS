...
class HomeViewController: UIViewController, SideMenuItemContent {
    ...
    func configTableView() {
        ...
        tableView.register(
            UINib(nibName: String(describing: ItemsTableViewCell.self), bundle: Bundle.main),
            forCellReuseIdentifier: String(describing: ItemsTableViewCell.self))
    }
    ...
    func configSearch() {
        searchController = AZSearchViewController(
            cellReuseIdentifier: String(describing: SearchResultTableViewCell.self),
        ...
        searchController.navigationBarClosure = { bar in
            ...
            let blue: UIColor =  .kpBackground
            
            if #available(iOS 15.0, *) {
                let appearance = UINavigationBarAppearance()
                appearance.configureWithOpaqueBackground()   // removes transparency
                appearance.backgroundColor = blue           // set blue
            ...
            bar.tintColor = .white // buttons color
            
            
            
        }
        searchController.emptyResultCellTextColor = .kpOffWhite
        searchController.minimalCharactersForSearch = 0
...
//            barButtonSystemItem: .stop, target: self, action: #selector(closeSearchBar(_:)))
...
//        searchController.navigationItem.rightBarButtonItem = item
    ...
    @objc func tap(sender: KPGestureRecognizer) {
        ...
        if let indexPath = sender.collectionView?.indexPathForItem(
            at: sender.location(in: sender.collectionView)) {
            if let cell = sender.collectionView?.cellForItem(at: indexPath)
                as? ItemCollectionViewCell {
                if let image = cell.posterImageView.image, let wideImage = cell.posterWideImageView.image {
                    showDetailVC(with: sender.item!, andImage: image, wideImage: wideImage)
                }
            }
        }
    }
    
    func showDetailVC(with item: Item, andImage image: UIImage, wideImage: UIImage) {
        if let detailViewController = DetailViewController.storyboardInstance() {
            detailViewController.image = image
            detailViewController.wideImag = wideImage
...
extension HomeViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        tableView.dequeueReusableCell(
            withIdentifier: String(describing: ItemsTableViewCell.self), for: indexPath)
...
extension HomeViewController: AZSearchViewDataSource {
    ...
    func searchView(
        _ searchView: AZSearchViewController, tableView: UITableView,
        cellForRowAt indexPath: IndexPath
    ) -> UITableViewCell {
        let cell =
        tableView.dequeueReusableCell(withIdentifier: SearchResultTableViewCell.reuseIdentifier)
...
extension HomeViewController: AZSearchViewDelegate {
    ...
    func searchView(
        _ searchView: AZSearchViewController, didTextChangeTo text: String, textLength: Int
    ) {
        model.resultItems.removeAll()
        if textLength > searchController.minimalCharactersForSearch {
            gradientLoadingBar.show()
            searchController.emptyResultCellText = "загрузка..."
            model.loadSearchItems(
                text,
                { [weak self] _ in
    ...
    func searchView(
        _ searchView: UITableView, didSelectResultAt indexPath: IndexPath, object: AnyObject
    ) {
        if let cell = searchView.cellForRow(at: indexPath) as? SearchResultTableViewCell {
            if let image = cell.posterImageView.image, let posterImageWide = cell.widePosterImageView.image {
                self.searchController.dismiss(
                    animated: true,
                    completion: {
                        self.showDetailVC(with: object as! Item, andImage: image, wideImage: posterImageWide)
                    })
            }
        }
    }
